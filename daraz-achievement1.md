...
&emsp;&emsp;GIF 对比图为巴基斯坦同学在本地线上环境录制，因为优化前最早的页面已经无法打开，旧版 GIF 使用的是经过第一波优化后的 4200ms 版本，而非最早的 6500ms 版本，不过对比依然明显，旧版跳转进首页会白屏等待好几秒钟，而新版的是瞬间打开
...
&emsp;&emsp;最终采用 React18 + Vite + Webpack 进行项目开发和构建，并联合后端采用一个主接口的 MTOP 请求方式来减少请求嵌套
...
&emsp;&emsp;和当地同学再次远程连线，查看了几个旧 dada 平台的页面，坐实了这个可怕的想法，但我仍然不敢相信，Daraz 拥有上百万的用户，居然没有在本地部署 CDN 服务器，最近的 CDN 服务器远在新加坡，每个请求都跋山涉水横跨多个国家的从巴基斯坦跑到新加坡，再带着请求到的信息从新加坡跑回来，这要是不慢那才是怪事了
...
&emsp;&emsp;事后来看，如果排除第二波的优化效果，第三波的优化效果约在 1500ms，远超了预期，看这样子，很多人都卡在了第一个请求的 html 文件上
...
&emsp;&emsp;用户很可能也是这么想的，首页的结构很早就加载出来了，但是首页所需要的数据种类很多，要拉取很多的大盘数据，相比普通页面加载确实会花费更多的时间

&emsp;&emsp;一旦问题从首页本身渲染的慢转变为拉取数据慢，涉及到用户自身个体差异，并且用户开始理解慢的原因时，用户就会变得宽容起来

&emsp;&emsp;因此我们只要将网站加载和请求数据在用户心中分离开，并且减少网站加载部分的时间，就可以让用户体验更好一些
...
&emsp;&emsp;这里由于没办法直接接触到真实用户的电脑，本地同学的网络环境在当地也算是高级写字楼，和真实的用户环境仍然有很大的差别（例如网络波动大，网络带宽小，农村晚上直接断电断网等等），所以肯定还有很多优化的空间未曾发现
